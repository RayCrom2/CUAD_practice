...
GOVERNING_LAW_TOOL = {
    "name": "report_governing_law",
    "description": "Report whether a governing law clause was found in the contract text, and what it says.",
    "input_schema": {
        "type": "object",
        "properties": {
            "found": {
                "type": "boolean",
                "description": "True if a governing law clause is present in the provided text, false otherwise.",
            },
            "clause_text": {
                "type": "string",
                "description": (
                    "The exact governing law clause, copied verbatim character-for-character "
...
def extract_governing_law(client, context):
    ...
    Uses forced tool use rather than free-text sentinel parsing: the model
    must call report_governing_law with a typed {found: bool, clause_text:
    str} payload. There's no free-text wrapper for commentary to leak
    through, and no string-matching heuristic needed to detect a "not
    found" response -- `found` is read directly off the structured payload.
    """
    ...
    prompt = f"""You are reviewing a commercial contract. Find the GOVERNING LAW clause: a
    sentence that explicitly states which state's or country's SUBSTANTIVE LAW governs the
    interpretation of the agreement (e.g. "This Agreement shall be governed by the laws of
    the State of Delaware", "construed in accordance with the laws of England").

    Do NOT confuse this with a forum-selection, venue, or dispute-jurisdiction clause, which
    states WHERE disputes will be litigated or arbitrated rather than WHICH LAW applies
    (e.g. "any disputes shall be settled in a court in Florida", "the parties submit to the
    exclusive jurisdiction of the courts of New York", "disputes shall be resolved through
    arbitration in San Diego"). These are legally distinct -- a contract can have one, both,
    or neither. If the only law-related language you find is about where or how disputes are
    resolved, not which law substantively governs the agreement, set found to false.
    ...
    Call the report_governing_law tool with your finding. If you find the clause, copy it
    verbatim character-for-character from the contract -- do not paraphrase, summarize, or
    rephrase it in your own words, even if you are confident about the meaning.
    ...
    resp = client.messages.create(
        model=MODEL,
        max_tokens=300,
        tools=[GOVERNING_LAW_TOOL],
        tool_choice={"type": "tool", "name": "report_governing_law"},
    ...
    if tool_call is None:
        ...
        # if the model ends the turn some other way.
