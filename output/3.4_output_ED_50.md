# Phase 3.4 — Effective Date extraction, n=50 (2026-07-06)

**Result: ACCURACY 38/50 = 76%** | input tokens 451,558 | estimated cost $1.43

Matches the 3.3 headline. The single targeted fix in this version (TOC downgrade) verifiably
recovers contract #47 when it fires correctly, but an ordering issue in the weak-section
fallback means the downgraded entries can still surface first — see below.

## What changed from 3.3 — table-of-contents tier downgrade

Some contracts have a table of contents whose lines match `HEADER_EFFECTIVE_DATE_PATTERNS`
(e.g., `"3.   EFFECTIVE DATE AND DURATION     5"`). These lines are valid section headers
and were being promoted to tier-2, crowding out the actual clause sections which held no
effective-date keywords in their headers and were therefore tier-0 and dropped.

Two signals reliably identify a TOC entry vs. a real section:

1. **Section body smaller than `_MIN_TIER2_SECTION_CHARS` (400 chars)** — a real clause
   section cannot fit in under 400 characters; a TOC line with a trailing sentence can.
2. **Trailing page number in the header** — `_TOC_PAGE_RE = r'\s+\d{1,3}\s*$'` matches
   `"3.   EFFECTIVE DATE AND DURATION     5"`.

The fix lives in `build_candidate_snippets()`, applied after scanning and before the
strong/weak split:
...
Downgrading to tier-0 (rather than dropping entirely) preserves the sections as a last-resort
fallback when no stronger signal exists — consistent with how the rest of the tier system works.

## Contract #47 (AzulSa) — partial fix

Before this change, the two TOC entries for contract #47 were selected as the entire candidate
context (38 chars + 275 chars = ~320 chars sent), and the model returned the section header
text verbatim rather than a date clause.

The downgrade correctly removes them from `strong`. However, the downgraded entries remain
in `section_order` (they had keyword matches during the scan) and appear early in that list
because the TOC precedes the actual content. This means `weak[:max_sections]` still selects
them as the first three weak candidates when `strong` is empty.

In the run that confirmed the fix (2nd run of 3.4), the section ordering happened to include
the real clause section in the first three weak slots, #47 passed. In this documented run the
ordering placed the downgraded TOC entries first again, #47 missed. The fix is structurally
correct but the fallback ordering needs a follow-up: downgraded sections should sort after
non-downgraded weak sections rather than by first-keyword-match order.

## Remaining misses — categorized

**Scorer-vs-substance gaps (5) — model substantively correct, different sentence than CUAD:**
- #6 (Veoneer): model finds execution date; gold is conditional future-effective clause
- #7 (DovaPharmaceuticals): model finds preamble date; gold is a Definitions cross-reference
- #10 (DominiAdvisor): model finds signing-date sentence; gold is "first above written" clause
- #11 (CerenceInc): model finds preamble date; gold is a counterpart-execution clause
- #24 (TuniuCorp): model finds entry date; gold is "when authorized representatives sign" clause

**Windowing misses (2):**
- #25 (CyberianOutpost): model finds execution date Jan 26; gold is Term commencement Feb 1 (same single section)
- #26 (SenmiaoTechnology): windowing finds procedural effectiveness clause; bare date "December 17, 2018" is in an un-windowed paragraph

**Section selection drops gold (2):**
- #43 (WPP): gold lives under header "JOHN ROGERS" (employment agreement, person name — no clause keyword); selected sections contain nothing relevant
- #48 (DragonSystems): 684-char tier-2 section (above 400 threshold, no trailing number) still selected; gold is in tier-0 "14. INSURANCE"

**Structural / un-fixable (2):**
- #40 (Principal Life): blank-date template (`"this ______ day of ________, 2013"`) — no keyword can match a redacted placeholder
- #44 (Waste2Energy): gold is `"Commencement Date means the date of this Agreement"` — "Commencement Date" is not in EFFECTIVE_DATE_PATTERNS; model finds signing-date sentence instead

## Next step

The `weak[:max_sections]` ordering issue (#47) can be resolved by separating the
downgraded-but-formerly-tier-2 sections from organic tier-0 sections in the fallback
selection order, so downgraded entries are tried last. This is a follow-up fix for 3.5.

## Raw run output, n=50

```
$ python3 phase3.4_effective_date.py "/Users/raymondcromwell/Downloads/data/CUADv1.json" --n 50 --show-usage --input-price-per-1m 3.0 --output-price-per-1m 15.0
...
[ 6/50] (contract #6) MISS VEONEER,INC_02_21_2020-EX-10.11-JOINT VENTURE...
        ...
        pred matched patterns: ['effective date', 'effective as of', 'made and entered into', '\\b(?:January|...|December)\\s+\\d{1,2},?\\s+\\d{4}\\b']
        src   : section-match
...
[ 7/50] (contract #7) MISS DovaPharmaceuticalsInc_20181108_10-Q_EX-10.2_...
        ...
        pred matched patterns: ['effective date', 'dated as of', '\\b(?:January|...|December)\\s+\\d{1,2},?\\s+\\d{4}\\b']
        src   : section-match
...
[10/50] (contract #10) MISS DOMINIADVISORTRUST_02_18_2005-EX-99.(H)(2)-SP...
        ...
        pred matched patterns: ['dated as of', '\\b(?:January|...|December)\\s+\\d{1,2},?\\s+\\d{4}\\b']
        src   : section-match
...
[25/50] (contract #25) MISS CYBERIANOUTPOSTINC_07_09_1998-EX-10.13-PROMOT...
        gold  : 'The term of this Agreement (the "Term") will begin on      February 1, 1998 and end on the first anniversary...'
...
[47/50] (contract #47) MISS AzulSa_20170303_F-1A_EX-10.3_9943903_EX-10.3_...
        ...
        usage : in=1312 out=67 total=2129
...
FALLBACKS: 0/50
TOKENS: input=451558 output=5192 total=456750
AVG INPUT TOKENS / CONTRACT: 9031.2
AVG INPUT TOKENS / SECTION MATCH: 9031.2
AVG INPUT TOKENS / FALLBACK: 0.0
CONTEXT CHARS: full=2520344 snippets=1739369
ESTIMATED COST: $1.4326
==================================================
```
